...
import type { TrainingConfig, BaseTrainingConfig } from './training-config.types';
...
export class TrainingConfigValidator {
  ...
  private validateModel(model: any, errors: ValidationError[], warnings: ValidationWarning[]): void {
  ...
  private validateTokenizer(tokenizer: any, errors: ValidationError[], warnings: ValidationWarning[]): void {
  ...
  private validateTraining(training: any, errors: ValidationError[], warnings: ValidationWarning[]): void {
    if (!training.method) {
      errors.push({
  ...
  private validateData(data: any, errors: ValidationError[], warnings: ValidationWarning[]): void {
    if (!data.strategy) {
      errors.push({
        field: 'data.strategy',
