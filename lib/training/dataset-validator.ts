    ...
    try {
      ...
      if (parsed.examples && Array.isArray(parsed.examples)) {
        ...
        for (let i = 0; i < parsed.examples.length; i++) {
          ...
          const humanMsgs = conversation.conversations.filter((t: any) => t.from === 'human' || t.from === 'user');
          const gptMsgs = conversation.conversations.filter((t: any) => t.from === 'gpt' || t.from === 'assistant');

          totalInputLen += humanMsgs.reduce((sum: number, t: any) => sum + t.value.length, 0);
          totalOutputLen += gptMsgs.reduce((sum: number, t: any) => sum + t.value.length, 0);
          validExamples++;
        }
