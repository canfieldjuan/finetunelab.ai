...
async def reconnect_orphaned_training_jobs():
    ...
    try:
        ...
        # DATABASE CLEANUP: Mark stale jobs as failed (runs regardless of process detection)
        logger.info("[Reconnect] Checking database for stale running/pending jobs...")

        if supabase:
            try:
                # Query database for ALL jobs in running or pending status
                db_response = supabase.table('local_training_jobs').select('id, status, updated_at, model_name').in_('status', ['running', 'pending']).execute()
                ...
                if db_response.data:
                    ...
                    for db_job in db_response.data:
                        ...
                        # Skip jobs that are in the reconnected jobs dict (they're active)
                        if job_id in jobs:
                            logger.debug(f"[Reconnect] Skipping {job_id[:8]}... - already reconnected")
                            continue
                        ...
                        if updated_at_str:
                            try:
                                ...
                                if time_diff > 600:
                                    ...
                                    logger.warning(f"[Reconnect] Found stale job: {job_id[:8]}... ({db_job['status']}, {int(time_diff)}s since update)")
                            except Exception as e:
                                logger.warning(f"[Reconnect] Error parsing timestamp for {job_id}: {e}")

                    # Mark stale jobs as failed in database
                    if stale_jobs:
                        logger.info(f"[Reconnect] Marking {len(stale_jobs)} stale job(s) as failed...")
                        ...
                        for stale_job in stale_jobs:
                            ...
                            try:
                                ...
                                if update_response.data:
                                    logger.info(f"[Reconnect] ✓ Marked {job_id[:8]}... as failed")
                                else:
                                    logger.warning(f"[Reconnect] Failed to update {job_id[:8]}... in database")

                            except Exception as e:
                                logger.error(f"[Reconnect] Error updating job {job_id}: {e}")
                    else:
                        logger.info("[Reconnect] No stale jobs found in database")
                else:
                    logger.info("[Reconnect] No running/pending jobs found in database")

            except Exception as e:
                logger.error(f"[Reconnect] Error checking database for stale jobs: {e}", exc_info=True)
        else:
            logger.warning("[Reconnect] Supabase not available - cannot check for stale jobs")
