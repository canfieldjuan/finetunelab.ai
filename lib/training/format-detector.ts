  ...
  try {
    ...
    if (lines.length > 1) {
      ...
      try {
        ...
        if ('conversations' in firstLine) {
          return {
            format: 'sharegpt',
            confidence: 'high',
            details: {
              hasSystemField: 'system' in firstLine,
        ...
        if ('prompt' in firstLine && 'chosen' in firstLine && 'rejected' in firstLine) {
          return {
            format: 'dpo',
            confidence: 'high',
            details: {
        ...
        if ('prompt' in firstLine && 'response' in firstLine) {
          return {
            format: 'rlhf',
            confidence: 'high',
            details: {
        ...
        if ('role' in firstLine && 'content' in firstLine) {
          return {
            format: 'chatml',
            confidence: 'medium',
            details: {
...
function detectSingleJsonFormat(parsed: unknown): FormatDetectionResult {
  ...
  if (typeof parsed === 'object' && parsed !== null && !Array.isArray(parsed) && 'examples' in parsed) {
    ...
    if (Array.isArray(examples) && examples.length > 0) {
      ...
      if ('conversations' in firstExample) {
        return {
          format: 'sharegpt-examples',
          confidence: 'high',
          details: {
            hasSystemField: 'system' in firstExample,
            hasConversationsField: true,
  ...
  if (typeof parsed === 'object' && parsed !== null && !Array.isArray(parsed) && 'conversations' in parsed) {
    const obj = parsed as Record<string, unknown>;
  ...
  if (typeof parsed === 'object' && parsed !== null && !Array.isArray(parsed) && 'messages' in parsed) {
  ...
  if (typeof parsed === 'object' && parsed !== null && !Array.isArray(parsed) && 'prompt' in parsed && 'chosen' in parsed && 'rejected' in parsed) {
    const sampleKeys = Object.keys(parsed);
  ...
  if (typeof parsed === 'object' && parsed !== null && !Array.isArray(parsed) && 'prompt' in parsed && 'response' in parsed) {
    const sampleKeys = Object.keys(parsed);
...
function detectConversationFormat(conversations: Array<Record<string, unknown>>): 'from-value' | 'role-content' {
  ...
  if ('from' in first && 'value' in first) {
    return 'from-value';
  }
  if ('role' in first && 'content' in first) {
    return 'role-content';
