...
def quantize_gguf(input_path: Path, output_path: Path, quantization: str) -> bool:
    """
    Quantize a GGUF file using llama-quantize binary
    ...
    try:
        ...
        if not llama_quantize_path or not Path(llama_quantize_path).exists():
            print(f"[GGUF Converter] ⚠ llama-quantize not found (quantization unavailable)")
            print(f"[GGUF Converter] Using F16 GGUF (recommended for development)")
        ...
        print(f"[GGUF Converter] Running quantization: {quantization}")
        ...
        if result.returncode == 0:
            print(f"[GGUF Converter] ✓ Quantization successful")
            return True
        else:
            print(f"[GGUF Converter] ⚠ Quantization failed:")
