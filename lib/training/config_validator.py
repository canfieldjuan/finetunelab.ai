...
def validate_dataset_format(dataset: List[Dict[str, Any]], data_strategy: str = "standard") -> Tuple[bool, List[str]]:
    ...
    # RELAXED VALIDATION: Accept normalized format
    # Datasets are now pre-normalized at upload, so messages format is expected
    if has_messages:
        # Valid normalized chat format
    ...
    if not has_messages and not has_text:
        raise ValidationError(
            "Dataset examples must have either 'messages' field (for chat) or 'text' field (for standard). "
