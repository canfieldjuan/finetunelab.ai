...
export class SearchService {
  ...
  // Phase 3: in-memory provider telemetry
  private readonly telemetry = new Map<string, { latencies: number[]; successes: number; errors: number }>();
  ...
  async search(
    ...
    for (const providerName of providersToTry) {
      ...
      try {
        ...
        this.recordProviderResult(providerName, duration, true);
        if (this.config.logResults) {
        ...
        lastError = error;
        this.recordProviderResult(providerName, 0, false);
  ...
  // ================================
  private recordProviderResult(name: string, latencyMs: number, success: boolean): void {
    const rec = this.telemetry.get(name) || { latencies: [], successes: 0, errors: 0 };
    if (success) rec.successes += 1; else rec.errors += 1;
    if (success && latencyMs > 0) {
      rec.latencies.push(latencyMs);
      if (rec.latencies.length > 50) rec.latencies.splice(0, rec.latencies.length - 50);
    }
    this.telemetry.set(name, rec);

    // Persist to DB (fire-and-forget) when enabled and service client is available
    if (process.env.SEARCH_TELEMETRY_PERSIST === 'true' && supabaseAdmin) {
      void Promise.resolve(
        supabaseAdmin
          .from('web_search_telemetry')
          .insert({ provider: name, latency_ms: latencyMs, success })
      )
        .then(({ error }) => {
          if (error && this.config.logResults) {
            console.warn('[WebSearch] Telemetry persist error (ignored):', error.message);
          }
        })
        .catch((e: unknown) => {
          if (this.config.logResults) {
            console.warn('[WebSearch] Telemetry persist exception (ignored):', e);
          }
        });
    }
  }
  ...
  private getProviderLatencyStats(name: string): { p50: number; p95: number } {
    const rec = this.telemetry.get(name);
    if (!rec || rec.latencies.length === 0) return { p50: Number.POSITIVE_INFINITY, p95: Number.POSITIVE_INFINITY };
