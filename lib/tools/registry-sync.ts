...
export interface ExistingRegistryToolRow {
...
export function mergeRegistryToolSeedRow(
  row: RegistryToolSeedRow,
  existing?: ExistingRegistryToolRow | null
): RegistryToolSeedRow {
  return {
    ...row,
    is_enabled: typeof existing?.is_enabled === 'boolean' ? existing.is_enabled : row.is_enabled,
  };
}
