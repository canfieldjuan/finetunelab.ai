...
class ModelManagerService {
  ...
  async getModelConfig(modelId: string, userId?: string): Promise<ModelConfig | null> {
    try {
      ...
      const isLocalProvider = localProviders.includes(data.provider.toLowerCase());

      // Priority 1: Use model's own API key if present
      if (data.api_key_encrypted) {
        apiKey = decrypt(data.api_key_encrypted);
        console.log('[ModelManager] ✓ Using model-specific API key');
      }
      // Priority 2: Look up provider secret if userId provided (skip for local providers)
      else if (userId && !isLocalProvider) {
        console.log('[ModelManager] No model-specific key, looking up provider secret for:', data.provider);
        try {
          ...
          if (providerKey) {
            apiKey = providerKey;
            console.log('[ModelManager] ✓ Using provider secret for:', data.provider);
          } else {
            console.warn('[ModelManager] ✗ No provider secret found for:', data.provider, '- API calls will fail with 401');
          }
        } catch (error) {
          console.error('[ModelManager] ✗ Failed to lookup provider secret:', error);
        ...
        console.warn('[ModelManager] ✗ No userId provided and model has no API key - API calls will fail with 401');
      }
