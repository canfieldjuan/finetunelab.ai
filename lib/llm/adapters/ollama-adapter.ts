...
export class OllamaAdapter extends BaseProviderAdapter {
  ...
  async parseResponse(response: Response, responseBody: any): Promise<AdapterResponse> {
    ...
    const content = responseBody.message?.content || responseBody.response || '';

    // Ollama doesn't always provide token counts
    // Estimate if not provided
    const usage = responseBody.prompt_eval_count || responseBody.eval_count
      ? {
          input_tokens: responseBody.prompt_eval_count || 0,
          output_tokens: responseBody.eval_count || 0,
        }
      : undefined;
