    ...
    const fetchInitialData = async () => {
      try {
        ...
        if (metricsError) {
          ...
          console.log('[TrainingRealtime] Loaded', existingMetrics.length, 'existing metrics');
          console.log('[TrainingRealtime] Steps:', existingMetrics.map(m => m.step));
      ...
      const channel = supabase
        ...
        .on(
          ...
          (payload) => {
            ...
            setMetrics((prevMetrics) => [...prevMetrics, payload.new as MetricPoint]);
          }
        )
