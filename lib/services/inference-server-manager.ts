...
export class InferenceServerManager {
  ...
  async startVLLM(
    ...
    try {
      ...
      // Resolve Python executable (Linux focused, no Windows fallback)
      const pythonPath = resolvePythonExecutable();
      console.log('[InferenceServerManager] Final python selection:', pythonPath);
  ...
  private async checkOllamaRunning(): Promise<boolean> {
    try {
      ...
      console.log('[InferenceServerManager] Ollama not running on port 11434');
      return false;
    }
  }
  ...
  async stopServer(serverId: string, userId: string | null): Promise<void> {
    ...
    await new Promise<void>((resolve) => {
      const timeout = setTimeout(() => {
        ...
        if (!process.killed) {
          console.warn('[InferenceServerManager] Force killing server:', serverId);
