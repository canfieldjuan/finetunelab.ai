...
import { LocalProviderConfig, ProviderStatus, ProviderConnection } from '@/lib/training/training-config.types';
    ...
    try {
      ...
      try {
        if (request && request.config) {
          ...
          const normalized = normalizeForBackend(request.config as any);
