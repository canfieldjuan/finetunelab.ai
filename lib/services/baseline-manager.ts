...
export class BaselineManager {
   ...
   */
  private mapBaseline(data: any): Baseline {
    return {
      id: data.id,
      modelName: data.model_name,
      version: data.version,
      metricName: data.metric_name,
      metricCategory: data.metric_category,
...
export function getBaselineManager(): BaselineManager {
  if (!baselineInstance) {
    ...
    if (!supabaseUrl || !supabaseKey) {
      throw new Error('Missing Supabase configuration for BaselineManager');
    }

    baselineInstance = new BaselineManager(supabaseUrl, supabaseKey);
  }
