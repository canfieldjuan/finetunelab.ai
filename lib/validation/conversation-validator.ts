...
export function validateConversation(
  ...
  if (conversation.messages) {
    for (const message of conversation.messages) {
      ...
      sanitizedMessages.push(messageResult.sanitizedData);
    }
  }
  ...
  try {
    ...
    if (typeof value === 'string') {
      ...
      try {
        ...
        errors.push(`Failed to parse JSON: ${parseError.message}`);
        healthScore = 0;
    ...
    errors.push(`Unexpected error validating JSON: ${error.message}`);
    return {
      isValid: false,
      errors,
      warnings,
