...
export class CitationsService {
  ...
  async saveCitations(
    ...
    const citationRecords: CitationInsert[] = citations.map((c: any) => ({
   ...
   */
  async getCitations(messageId: string): Promise<any[]> {
    const { data, error } = await supabase
      .from('citations')
      .select('*')
      .eq('message_id', messageId)
   ...
   */
  async getCitationsWithDocuments(messageId: string): Promise<any[]> {
    const { data, error } = await supabase
      .from('citations')
      .select(`
        *,
