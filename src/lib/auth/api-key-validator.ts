...
const RATE_LIMIT_PER_KEY = 100; // requests per minute
...
export function checkRateLimit(apiKeyHash: string): RateLimitResult {
  ...
  // Check if exceeded
  const allowed = limitEntry.count <= RATE_LIMIT_PER_KEY;
  const remaining = Math.max(0, RATE_LIMIT_PER_KEY - limitEntry.count);
  
  if (!allowed) {
    console.warn('[Rate Limit] Exceeded for key:', apiKeyHash.substring(0, 20) + '...');
    console.warn('[Rate Limit] Count:', limitEntry.count, 'Limit:', RATE_LIMIT_PER_KEY);
  }
