...
export class DocumentService {
  ...
  /**
   * Process text as single episode (like promote conversation)
   * This is much faster than chunking for typical documents
   */
  private async processAsSingleEpisode(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        console.log(`[GraphRAG] Attempt ${attempt}/${maxRetries}: Adding document as single episode`);
