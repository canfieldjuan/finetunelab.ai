...
export async function getUserProfileContext(
  ...
  try {
    const { data, error } = await supabase
      ...
      .select('full_name, display_name, timezone, locale, role, company, preferred_response_length, expertise_level')
      .eq('user_id', userId)
      .single();
    ...
    if (!data) {
      console.warn('[ContextProvider] No profile found for user:', userId);
      return null;
    }
...
export async function getFeatureFlagsContext(
  ...
  try {
    ...
    if (subError) {
      console.error('[ContextProvider] Error fetching subscription:', subError);
    }

    const subscriptionPlans = subscription?.subscription_plans as any;
    const planName = subscriptionPlans?.name || 'free';
    const planLimits = subscriptionPlans?.limits || {};
    const apiCallsLimit = planLimits.api_calls_per_month || 1000;
