...
export async function GET(req: NextRequest) {
  try {
    ...
    let query = supabase
      .from('llm_traces')
      .select('id, trace_id, span_name, operation_type, status, start_time, duration_ms, model_name, model_provider, conversation_id, message_id, session_tag, error_message, input_tokens, output_tokens, total_tokens, cost_usd, ttft_ms, tokens_per_second, api_endpoint, api_base_url, request_headers_sanitized, provider_request_id, queue_time_ms, inference_time_ms, network_time_ms, streaming_enabled, chunk_usage, context_tokens, retrieval_latency_ms, groundedness_score, response_quality_breakdown, warning_flags, reasoning', { count: 'exact' })
      .eq('user_id', user.id);
