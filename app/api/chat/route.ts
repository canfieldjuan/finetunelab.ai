...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    try {
      ({ messages, memory, tools: rawTools, conversationId, modelId, widgetSessionId, forceNonStreaming, runId, benchmarkId, userId: requestUserId, contextInjectionEnabled, enableThinking } = await req.json());
    } catch (jsonError) {
    ...
    const toolCallHandler = async (toolName: string, args: Record<string, unknown>) => {
      ...
      try {
        if (toolName === 'web_search' && isWebSearchToolPayload(result.data)) {
          ...
          if (status === 'deep_research_started' && jobId) {
