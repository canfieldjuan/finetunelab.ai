  ...
  try {
    ...
    const serversWithStatus = await Promise.all(
      (servers || []).map(async (server) => {
        ...
        // If server claims to be running, verify the process is actually alive
        if (server.status === 'running' && server.process_id) {
          try {
            ...
            console.log(`[ServerStatus] Process ${server.process_id} for server ${server.id} is dead, marking as stopped`);
