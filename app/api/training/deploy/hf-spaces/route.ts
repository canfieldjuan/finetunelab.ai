...
export async function GET(request: NextRequest) {
  try {
    ...
    const budgetCheck = hfSpacesService.checkBudgetThreshold(
      ...
      deploymentConfig?.alert_threshold || 80
    );
    ...
    if (status.status === 'running' && !deploymentRecord.started_at) {
      updateData.started_at = new Date().toISOString();
    }

    // Set completed_at if training finished
    if (['completed', 'failed', 'stopped'].includes(status.status) && !deploymentRecord.completed_at) {
