    ...
    const datasetsWithSize = await Promise.all(
      (datasets || []).map(async (dataset) => {
        try {
          ...
          const size = (fileInfo as any).metadata?.size ||
                       (fileInfo as any).metadata?.contentLength ||
                       0;
          console.log(`[DatasetsAPI] Dataset ${dataset.id} size: ${size} bytes (from metadata)`);
          console.log(`[DatasetsAPI] File metadata:`, JSON.stringify((fileInfo as any).metadata || {}));
