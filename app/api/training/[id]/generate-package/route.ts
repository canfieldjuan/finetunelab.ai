...
async function getCompatibleMethods(
  ...
  console.log('[GeneratePackageAPI] Found datasets:', datasets.map((d: any) => ({ format: d.format })));

  const allMethods: TrainingMethod[] = ['sft', 'dpo', 'rlhf'];
  const compatibleMethods = allMethods.filter(method => {
    const isCompatible = datasets.every((dataset: any) => {
      const compatible = isFormatCompatible(dataset.format as any, method);
...
export async function POST(
  ...
  try {
    ...
    let gistUrls: Record<string, string> = {};
