 ...
 * 
 * Evaluates AI messages using GPT-4 or Claude as judges
 * Competes with LangSmith's evaluation capabilities
 */
...
async function handleSingleEvaluation(
  ...
  // Initialize judge
  const judge = new LLMJudge(request.judge_model || 'gpt-4-turbo');
...
async function handleBatchEvaluation(
  ...
  const batchRequests = messages.map((msg: { id: string; content: string }) => ({
    message_id: msg.id,
  ...
  const summary = {
    ...
    average_score: allJudgments.reduce((sum, r) => sum + r.score, 0) / allJudgments.length,
    average_confidence:
