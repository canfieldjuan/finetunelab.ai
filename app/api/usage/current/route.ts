...
import type { GetUsageResponse, UsageMetrics, PlanLimits } from '@/lib/subscriptions/types';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const usage: UsageMetrics = {
      api_calls: usageSummary?.find((u: any) => u.metric_type === 'api_call')?.total_value || 0,
      tokens: usageSummary?.find((u: any) => u.metric_type === 'token_usage')?.total_value || 0,
      storage_mb: usageSummary?.find((u: any) => u.metric_type === 'storage_mb')?.total_value || 0,
      training_jobs: usageSummary?.find((u: any) => u.metric_type === 'training_job')?.total_value || 0,
      models: usageSummary?.find((u: any) => u.metric_type === 'model_created')?.total_value || 0,
    };

    // Extract plan limits (type assertion needed due to Supabase join structure)
    const limits: PlanLimits = (subscription as any).plan.limits;
