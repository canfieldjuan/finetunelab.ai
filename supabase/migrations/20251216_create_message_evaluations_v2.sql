-- Migration: Create Message Evaluations Table (Safe Version)
-- Created: 2025-12-16
-- Purpose: Store user ratings and evaluations of LLM message quality

-- Drop existing table if it exists (to start fresh)
DROP TABLE IF EXISTS public.message_evaluations CASCADE;
...
CREATE INDEX idx_message_evaluations_user_id
  ON public.message_evaluations(user_id);

CREATE INDEX idx_message_evaluations_message_id
  ON public.message_evaluations(message_id);

CREATE INDEX idx_message_evaluations_created_at
  ON public.message_evaluations(created_at DESC);

CREATE INDEX idx_message_evaluations_rating
  ON public.message_evaluations(rating);

CREATE INDEX idx_message_evaluations_success
