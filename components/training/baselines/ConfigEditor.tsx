...
import { Save, X, AlertTriangle, CheckCircle2, Loader2, Server } from 'lucide-react';
import type { TrainingConfig, TrainingProvider, LocalProviderConfig } from '@/lib/training/training-config.types';
import { supabase } from '@/lib/supabaseClient';
import { buildUiConfig, validateBeforeUse } from '@/lib/training/config-builder';
import { DistributedTrainingConfig } from './DistributedTrainingConfig';
...
export function ConfigEditor({ 
  ...
  // Provider state (Phase 2.3: Local Training Connection)
    ...
    try {
      ...
      const requestBody = isUpdate
        ...
        : {
            ...
            template_type: (builtConfig as any).template_type || 'sft',
            config_json: builtConfig
          };
      ...
      console.log('[ConfigEditor] Sending new parameters:', {
        lr_scheduler_type: (builtConfig.training as any).lr_scheduler_type,
        warmup_ratio: (builtConfig.training as any).warmup_ratio,
        save_steps: (builtConfig.training as any).save_steps,
        save_total_limit: (builtConfig.training as any).save_total_limit,
        evaluation_strategy: (builtConfig.training as any).evaluation_strategy,
        eval_batch_size: (builtConfig.training as any).eval_batch_size,
        packing: (builtConfig.training as any).packing,
        group_by_length: (builtConfig.training as any).group_by_length,
        dataloader_num_workers: (builtConfig.training as any).dataloader_num_workers,
        dataloader_prefetch_factor: (builtConfig.training as any).dataloader_prefetch_factor,
        dataloader_pin_memory: (builtConfig.training as any).dataloader_pin_memory,
      });
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-4 border-b pb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
            <div className="space-y-2">
              ...
              <input
                type="number"
                value={(config.training as any).dataloader_num_workers ?? 4}
            ...
            <div className="space-y-2">
              ...
              <input
                type="number"
                value={(config.training as any).dataloader_prefetch_factor ?? 2}
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                <input
                  ...
                  checked={Boolean((config.training as any).dataloader_pin_memory ?? true)}
        ...
        <div className="space-y-4 border-b pb-6">
          ...
          <div className="space-y-2">
            ...
            <input
              type="number"
              value={(config.training as any).eval_batch_size || ''}
          ...
          <div className="space-y-2">
            <label className="flex items-center space-x-2">
              <input
                type="checkbox"
                checked={(config.training as any).pretokenize ?? true}
          ...
          <div className="space-y-2">
            <label className="flex items-center space-x-2">
              <input
                type="checkbox"
                checked={(config.training as any).use_streaming ?? false}
          ...
          <div className="space-y-2">
            ...
            <input
              type="number"
              value={(config.training as any).logging_steps ?? 50}
          ...
          <div className="space-y-2">
            ...
            <input
              type="number"
              value={(config.training as any).eval_steps ?? 100}
          ...
          <div className="space-y-2">
            ...
            <select
              value={(config.training as any).lr_scheduler_type ?? 'cosine'}
          ...
          <div className="space-y-2">
            ...
            <input
              type="number"
              step="0.01"
              value={(config.training as any).warmup_ratio ?? ''}
          ...
          <div className="space-y-2">
            ...
            <input
              type="number"
              value={(config.training as any).save_steps ?? 500}
              onChange={(e) => updateTrainingField('save_steps', parseInt(e.target.value) || 500)}
              className="w-full px-3 py-2 border rounded-md"
              min="1"
            />
            <p className="text-xs text-gray-500">
              Save a checkpoint every N steps. Lower = more frequent saves (uses more disk space). Use 500-1000 for most cases.
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <input
              type="number"
              value={(config.training as any).save_total_limit ?? 3}
          ...
          <div className="space-y-2">
            ...
            <select
              value={(config.training as any).evaluation_strategy ?? 'steps'}
            ...
            <p className="text-xs text-gray-500">
              When to run evaluation on validation set. &quot;steps&quot; = use Eval Steps above, &quot;epoch&quot; = once per epoch, &quot;no&quot; = skip evaluation.
            </p>
          </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <input
                type="checkbox"
                id="packing"
                checked={Boolean((config.training as any).packing)}
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                <input
                  type="checkbox"
                  id="bf16_main"
                  checked={Boolean((config.training as any).bf16)}
                  onChange={(e) => {
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                <input
                  type="checkbox"
                  id="fp16_main"
                  checked={Boolean((config.training as any).fp16)}
                  onChange={(e) => {
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                <input
                  ...
                  checked={Boolean((config.training as any).gradient_checkpointing)}
              ...
              <div className="border-t pt-4 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <input
                      type="text"
                      value={(config.training.lora_config as any)?.target_modules?.join(', ') || 'q_proj, k_proj, v_proj, o_proj'}
                      onChange={(e) => updateTrainingField('lora_config', {
                        ...(config.training.lora_config as any || {}),
                    ...
                    <p className="text-xs text-gray-500">
                      ...
                      <strong>q_proj, k_proj, v_proj, o_proj:</strong> The attention mechanism (recommended - targets where the model &quot;focuses&quot;)<br/>
                      <strong>Why default is best:</strong> Attention layers learn most of the task-specific behavior<br/>
                      <strong>Advanced:</strong> Add &quot;gate_proj, up_proj, down_proj&quot; for even more adaptation (uses more memory)
                    </p>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <select
                      value={(config.training.lora_config as any)?.bias || 'none'}
                      onChange={(e) => updateTrainingField('lora_config', {
                        ...(config.training.lora_config as any || {}),
                        bias: e.target.value
                      })}
                  ...
                  <div className="space-y-2">
                    ...
                    <select
                      value={(config.training.lora_config as any)?.task_type || 'CAUSAL_LM'}
                      onChange={(e) => updateTrainingField('lora_config', {
                        ...(config.training.lora_config as any || {}),
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      type="text"
                      value={(config.training.lora_config as any)?.modules_to_save?.join(', ') || ''}
                      onChange={(e) => updateTrainingField('lora_config', {
                        ...(config.training.lora_config as any || {}),
              ...
              <div className="border-t pt-4 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      <input
                        type="checkbox"
                        id="load_in_4bit"
                        checked={(config.training.quantization as any)?.load_in_4bit ?? true}
                        onChange={(e) => updateTrainingField('quantization', {
                          ...(config.training.quantization as any || {}),
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      <input
                        type="checkbox"
                        id="load_in_8bit"
                        checked={(config.training.quantization as any)?.load_in_8bit ?? false}
                        onChange={(e) => updateTrainingField('quantization', {
                          ...(config.training.quantization as any || {}),
                  ...
                  <div className="space-y-2">
                    ...
                    <select
                      value={(config.training.quantization as any)?.bnb_4bit_quant_type || 'nf4'}
                      onChange={(e) => updateTrainingField('quantization', {
                        ...(config.training.quantization as any || {}),
                        bnb_4bit_quant_type: e.target.value
                      })}
                      className="w-full px-3 py-2 border rounded-md"
                      disabled={!(config.training.quantization as any)?.load_in_4bit}
                    >
                  ...
                  <div className="space-y-2">
                    ...
                    <select
                      value={(config.training.quantization as any)?.bnb_4bit_compute_dtype || 'bfloat16'}
                      onChange={(e) => updateTrainingField('quantization', {
                        ...(config.training.quantization as any || {}),
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      <input
                        type="checkbox"
                        id="double_quant"
                        checked={(config.training.quantization as any)?.bnb_4bit_use_double_quant ?? true}
                        onChange={(e) => updateTrainingField('quantization', {
                          ...(config.training.quantization as any || {}),
        ...
        <div className="space-y-4 border-b pb-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-lg font-semibold">7. Provider/Infrastructure</h3>
          </div>
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <select
                ...
                onChange={(e) => setSelectedProvider(e.target.value as TrainingProvider)}
                className="w-full px-3 py-2 border rounded-md"
              >
                <option value="local">Local Server (localhost:8000)</option>
                <option value="huggingface" disabled>HuggingFace (Coming Soon)</option>
                <option value="remote" disabled>Remote Server (Coming Soon)</option>
              </select>
              <p className="text-xs text-gray-500">
                <strong>Local Server:</strong> Run training on your own machine (requires local training server running)<br/>
                <strong>HuggingFace (Coming Soon):</strong> Train in HuggingFace cloud with AutoTrain<br/>
                <strong>Remote Server (Coming Soon):</strong> Connect to your own remote training infrastructure
              </p>
            </div>
            ...
            {selectedProvider === 'local' && (
              <div className="space-y-3 pl-4 border-l-2">
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-gray-500">
                    Default: http://localhost:8000 (must be running before starting training)
                  </p>
                </div>
                ...
                {connectionStatus === 'error' && (
                  <Alert variant="destructive">
                    <AlertTriangle className="h-4 w-4" />
                    <AlertDescription>
                      {connectionMessage || 'Failed to connect to local server'}
                    </AlertDescription>
                  </Alert>
                )}
              </div>
        ...
        {/* Distributed Training Configuration - Phase 3: Advanced Training Features */}
        <div className="space-y-4 border-t pt-4">
          <DistributedTrainingConfig
            value={(config.training as any).distributed}
            onChange={(distributedConfig) => {
              setConfig(prev => ({
                ...prev,
                training: {
                  ...prev.training,
        ...
        <div className="space-y-4 border-b pb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="space-y-2">
              ...
              <input
                type="text"
                value={(config.training as any).optim ?? 'paged_adamw_8bit'}
            ...
            <div className="space-y-2">
              ...
              <input
                type="number"
                step="0.0001"
                value={(config.training as any).weight_decay ?? 0.01}
            ...
            <div className="space-y-2">
              ...
              <input
                type="number"
                step="0.01"
                value={(config.training as any).max_grad_norm ?? 0.3}
            ...
            <div className="space-y-2">
              ...
              <input
                type="number"
                step="0.01"
                value={(config.training as any).adam_beta1 ?? 0.9}
            ...
            <div className="space-y-2">
              ...
              <input
                type="number"
                step="0.001"
                value={(config.training as any).adam_beta2 ?? 0.999}
            ...
            <div className="space-y-2">
              ...
              <input
                type="number"
                step="0.0000001"
                value={(config.training as any).adam_epsilon ?? 1e-8}
