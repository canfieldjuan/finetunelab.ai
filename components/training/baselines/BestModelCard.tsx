...
export function BestModelCard({ status }: BestModelCardProps) {
  ...
  const getEarlyStoppingMessage = () => {
    ...
    if (noImprovement === 0) {
      return { text: 'Model is improving!', color: 'text-green-700' };
    } else if (noImprovement < 3) {
      return { text: `${noImprovement} epoch(s) without improvement`, color: 'text-blue-700' };
    } else if (noImprovement < 5) {
      return { text: `Consider early stopping (${noImprovement} epochs)`, color: 'text-amber-700' };
    } else {
      return { text: `Training may have plateaued (${noImprovement} epochs)`, color: 'text-red-700' };
    }
  };
