 ...
 */
function validateStepData(stepId: StepId, data: any): ValidationResult {
...
export function useWorkflowState({
  ...
  const updateStepData = useCallback(<T,>(stepId: StepId, data: T) => {
    ...
    setState((prev) => {
      ...
      const stepState = updatedSteps[stepId] as StepState<T>;
  ...
  const completeStep = useCallback((stepId: StepId) => {
    ...
    setState((prev) => {
      const updatedSteps = { ...prev.steps };
      const stepState = updatedSteps[stepId];
