...
export function LocalPackageDownloader({
  ...
  const handleStartTraining = async () => {
    ...
    try {
      ...
      // Download dataset from Supabase if available
      let datasetContent = '';
      if (linkedDatasets.length > 0) {
        console.log('[LocalPackageDownloader] Downloading dataset from Supabase Storage...');
        const datasetPath = linkedDatasets[0].storage_path;
        ...
        try {
          ...
          if (downloadError || !fileData) {
            console.error('[LocalPackageDownloader] Failed to download dataset:', downloadError);
            throw new Error(`Failed to download dataset: ${downloadError?.message || 'Unknown error'}`);
          }

          // Check if dataset is gzip compressed (stored as .jsonl.gz)
          const isCompressed = datasetPath.endsWith('.gz');
          console.log('[LocalPackageDownloader] Dataset compressed:', isCompressed);
          
          if (isCompressed) {
            // Decompress gzipped data
            console.log('[LocalPackageDownloader] Decompressing gzipped dataset...');
            const decompressionStream = new DecompressionStream('gzip');
            const decompressedStream = fileData.stream().pipeThrough(decompressionStream);
            const decompressedBlob = await new Response(decompressedStream).blob();
            datasetContent = await decompressedBlob.text();
            console.log('[LocalPackageDownloader] Decompressed dataset, size:', datasetContent.length, 'bytes');
          } else {
            // Plain text dataset
            datasetContent = await fileData.text();
            console.log('[LocalPackageDownloader] Dataset downloaded, size:', datasetContent.length, 'bytes');
          }
        } catch (err) {
          console.error('[LocalPackageDownloader] Dataset download error:', err);
          throw new Error(`Dataset download failed: ${err instanceof Error ? err.message : 'Unknown error'}`);
        }
      }
      ...
      // Prepare training request with dataset content, not path
      const trainingRequest = {
        config: configData.config.config_json,
        dataset_content: datasetContent, // Send content instead of path
        execution_id: jobId,
        name: configName,
        user_id: userId,  // Pass user ID for database persistence
      };

      console.log('[LocalPackageDownloader] Calling training API on port 8000...');
      console.log('[LocalPackageDownloader] User ID:', userId);
