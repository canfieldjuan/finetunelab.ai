 ...
 */
export function useChat({ user, activeId, tools, enableDeepResearch, selectedModelId, contextTrackerRef, setContextUsage, isStreamingRef, researchProgress, setResearchProgress, activeResearchJob, setActiveResearchJob }: UseChatOptions) {
              ...
              if (selectedModelId !== '__default__' && activeId && isValidUUID) {
                ...
                if (activeContext) {
                  ...
                  supabase
                    ...
                    .then(({ error }) => {
                      if (error) {
                        log.error('useChat', 'Error saving context', {
                          error,
