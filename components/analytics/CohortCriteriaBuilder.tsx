  ...
  const buildCriteria = (list: Criterion[]) => {
    ...
    for (const criterion of list) {
      switch (criterion.type) {
        case 'signup_date':
          newCriteria.signup_date = criterion.config;
          break;
        case 'subscription_plan':
          newCriteria.subscription_plan = criterion.config;
          break;
        case 'total_conversations':
          newCriteria.total_conversations = criterion.config;
          break;
        case 'activity_level':
          newCriteria.activity_level = criterion.config;
          break;
        case 'last_active':
          newCriteria.last_active = criterion.config;
          break;
      }
    }
    ...
    switch (criterion.type) {
        ...
        return (
          <div className="flex gap-2">
            ...
            <input
              ...
              value={criterion.config[Object.keys(criterion.config)[0]] || 0}
              onChange={(e) => {
      ...
      case 'activity_level':
        return (
          <select
            className="border rounded px-2 py-1 text-sm"
            value={criterion.config || 'medium'}
            onChange={(e) => {
              updateCriterion(criterion.id, { config: e.target.value });
            }}
          >
        ...
        return (
          <div className="flex gap-2 items-center">
            ...
            <input
              ...
              value={criterion.config?.days_ago || 30}
              onChange={(e) => {
        ...
        return (
          <div className="flex gap-2">
            <input
              ...
              value={(criterion.config?.in || []).join(', ')}
              onChange={(e) => {
