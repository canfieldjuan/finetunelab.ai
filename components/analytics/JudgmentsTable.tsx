...
export function JudgmentsTable({ data, timeRange = '30d' }: JudgmentsTableProps) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Top Judgment Tags</CardTitle>
      </CardHeader>
      <CardContent>
        {topTags.length === 0 ? (
              ...
              <tbody>
                {topTags.map(tag => (
                  <tr key={tag.tag} className="border-b">
                    <td className="py-2 px-3">{tag.tag}</td>
          ...
          <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50" role="dialog" aria-modal="true">
            <div className="bg-white rounded-lg shadow-xl max-w-3xl w-full max-h-[80vh] overflow-hidden">
              <div className="p-4 border-b flex items-center justify-between">
                <div className="font-medium">Examples for: {modalTag}</div>
              ...
              <div className="p-4 space-y-3 overflow-y-auto max-h-[60vh]">
                ...
                {examples.map((ex) => (
                  <div key={`${ex.messageId}-${ex.createdAt}`} className="border rounded p-2">
                    ...
                    <div className="text-[11px] text-gray-500 mt-1">Tags: {ex.failureTags.join(', ') || '—'}</div>
                  </div>
                ))}
              </div>
