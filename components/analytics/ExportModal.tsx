...
export function ExportModal({ isOpen, onClose, data }: ExportModalProps) {
  ...
  const exportOptions: ExportOption[] = [
    ...
    },
    {
      ...
      available: data?.tokenUsage?.length > 0
    },
    {
      id: 'costTracking',
      label: 'Cost Tracking',
      description: 'Cost analysis and trends',
      available: data?.costTracking?.length > 0
    }
  ];
  ...
  const handleExport = () => {
    ...
    if (selectedFormat === 'csv') {
      ...
      switch (selectedSection) {
        case 'all':
          exportAllAnalyticsToCSV(data);
          break;
        case 'overview':
          exportOverviewToCSV(data.overview);
          break;
        case 'modelPerformance':
          exportModelPerformanceToCSV(data.modelPerformance);
          break;
        case 'sessionMetrics':
          exportSessionMetricsToCSV(data.sessionMetrics);
          break;
        case 'trainingEffectiveness':
          exportTrainingEffectivenessToCSV(data.trainingEffectiveness);
          break;
        case 'ratingDistribution':
          exportRatingDistributionToCSV(data.ratingDistribution);
          break;
        case 'tokenUsage':
          exportTokenUsageToCSV(data.tokenUsage);
          break;
        case 'costTracking':
          exportCostTrackingToCSV(data.costTracking);
          break;
      }
