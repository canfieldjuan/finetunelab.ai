...
import { PLANS, formatPrice, formatLimit, getYearlySavings, calculateTotalPrice, getSeatYearlySavings, type PlanTier } from '@/lib/pricing/config';
...
export function PricingTiers({ currentPlan, onUpgrade, onContactSales }: PricingTiersProps) {
  ...
  const seatOptions = [1, 2, 3, 5, 10, 25, 50];
            ...
            >
              ...
              <div className="mb-6">
                {plan.features.contactSales ? (
                  ...
                  <>
                    {/* Seat Selector (for Pro and Pro Plus) */}
                    {plan.pricing.seatPrice && (
                      <div className="mb-3">
                        ...
                        <select
                          value={selectedSeats[tier]}
                          onChange={(e) => setSelectedSeats({ ...selectedSeats, [tier]: Number(e.target.value) })}
                    ...
                    <div className="flex items-baseline gap-1">
                      <span className="text-3xl font-bold">
                        ${calculateTotalPrice(plan, billingInterval, selectedSeats[tier]).toLocaleString()}
                      </span>
                      <span className="text-muted-foreground">/{billingInterval === 'monthly' ? 'mo' : 'yr'}</span>
                    </div>

                    {/* Seat breakdown */}
                    {plan.pricing.seatPrice && selectedSeats[tier] > 1 && (
                      <p className="text-xs text-muted-foreground mt-1">
                        Base: ${price.toLocaleString()} + {selectedSeats[tier] - 1} seat
                        {selectedSeats[tier] > 2 ? 's' : ''} × $
                        {billingInterval === 'monthly' ? plan.pricing.seatPrice.monthly : plan.pricing.seatPrice.yearly}
                      </p>
                    )}
                      ...
                      <>
                        {savings > 0 && (
                          <p className="text-xs text-green-600 dark:text-green-400 mt-1">
                            Save ${savings.toLocaleString()}/year on base plan
                          </p>
                        )}
                        {plan.pricing.seatPrice && selectedSeats[tier] > 1 && (
                          <p className="text-xs text-green-600 dark:text-green-400">
                            + ${getSeatYearlySavings(plan, selectedSeats[tier]).toLocaleString()}/year on seats
                          </p>
                        )}
                      </>
                    )}
              ...
              {isCurrentPlan(tier) ? (
                ...
                <Button
                  onClick={() => onUpgrade(tier, billingInterval, selectedSeats[tier])}
                  className={`w-full ${
                    plan.features.popular
        ...
        <div className="overflow-x-auto">
          <table className="w-full border-collapse">
            ...
            <tbody className="divide-y">
              <tr>
                <td className="py-3 px-4 text-sm">API Calls/Month</td>
                {planOrder.map((tier) => (
                  <td key={tier} className="text-center py-3 px-4 text-sm">
                    {formatLimit(PLANS[tier].limits.api_calls_per_month)}
                  </td>
                ))}
              </tr>
              <tr>
              ...
              <tr>
                <td className="py-3 px-4 text-sm">Models</td>
                {planOrder.map((tier) => (
                  <td key={tier} className="text-center py-3 px-4 text-sm">
                    {formatLimit(PLANS[tier].limits.models_limit)}
                  </td>
                ))}
              </tr>
              <tr>
