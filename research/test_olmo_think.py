#!/usr/bin/env python3
"""Test allenai/olmo-3-32b-think with 5 prompts"""
...
Return ONLY a valid JSON array with exactly 5 objects. No markdown, just raw JSON."""
    ...
    try:
        ...
        # Calculate cost
        input_cost = (usage.prompt_tokens / 1_000_000) * 0.06
        output_cost = (usage.completion_tokens / 1_000_000) * 0.18
        ...
        try:
            ...
            # Save output
            output_file = Path(__file__).parent / "test_results" / f"olmo_think_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            ...
            # Save raw for debug
            debug_file = Path(__file__).parent / "test_results" / f"olmo_think_debug_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
            debug_file.parent.mkdir(exist_ok=True)
            with open(debug_file, 'w') as f:
                f.write(content)
            print(f"Raw saved to: {debug_file.name}")
