#!/usr/bin/env python3
"""Test anthropic/claude-sonnet-4.5 with 5 prompts"""
...
Return ONLY a valid JSON array with exactly 5 objects. No markdown, just raw JSON."""
    ...
    try:
        ...
        try:
            ...
            # Save output
            output_file = Path(__file__).parent / "test_results" / f"claude_sonnet45_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            ...
            # Save raw for debug
            debug_file = Path(__file__).parent / "test_results" / f"claude_sonnet45_debug_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
