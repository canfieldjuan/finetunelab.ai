...
/**
 * DAG Orchestrator Integration Test
 * 
 * Tests the DAG orchestrator with a simple pipeline
 * Run: node test-dag-orchestrator.js
 */

import DAGOrchestrator, { JobConfig, JobContext } from './lib/training/dag-orchestrator.js';
...
orchestrator.registerHandler('training', async (config, context) => {
...
orchestrator.registerHandler('preprocessing', async (config, context) => {
...
orchestrator.registerHandler('validation', async (config, context) => {
  context.log(`Validating: ${config.name}`);
  
  // Get output from previous job
  const trainingOutput = context.getJobOutput('train_model');
  context.log(`Training processed ${trainingOutput?.processedItems} items`);
...
orchestrator.execute('Test Pipeline', jobs, {
  parallelism: 2,
