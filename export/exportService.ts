  ...
  private async loadConversations(
    ...
    if (options.widgetSessionFilter && options.widgetSessionFilter !== 'all') {
      ...
      if (options.widgetSessionFilter === 'widget') {
        filteredConversations = conversations.filter((c: any) => c.is_widget_session === true);
      } else if (options.widgetSessionFilter === 'normal') {
        filteredConversations = conversations.filter((c: any) => c.is_widget_session === false);
      }
    ...
    const filteredConversationIds = filteredConversations.map((c: any) => c.id);
    ...
    const messagesByConv = (allMessages || []).reduce((acc: any, msg: any) => {
    ...
    return filteredConversations.map((conv: any) => {
      let messages: MessageData[] = messagesByConv[conv.id] || [];
